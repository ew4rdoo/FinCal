...
import { Calendar, TrendingUp, Bell, Star, ChevronLeft, ChevronRight, Search, X, Clock, Filter, Zap, Globe, Building2, Coins, BarChart3, ExternalLink, AlertCircle, FileText, BarChart2 } from 'lucide-react';
              ...
              return (
                <div key={ev.id} onClick={() => openEventModal(ev)} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '14px 16px', cursor: 'pointer', borderBottom: '1px solid #27272a' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 500, marginTop: 4 }}>{ev.ticker && <span style={{ color: '#6366f1' }}>${ev.ticker} </span>}{ev.title.substring(0, 25)}</div>
        ...
        <div onClick={() => setSelectedEvent(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#18181b', borderRadius: 16, border: '1px solid #27272a', maxWidth: 640, width: '100%', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid #27272a' }}>
              {[{ id: 'overview', label: 'Overview', icon: BarChart2 }, { id: 'research', label: 'Research', icon: FileText, count: researchLinks.length }].map(tab => (
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
              ...
              {activeTab === 'research' && (<>
                ...
                <div style={{ marginTop: 20, padding: 16, background: 'rgba(99,102,241,0.08)', borderRadius: 10, border: '1px solid rgba(99,102,241,0.2)' }}>
                  <div style={{ fontSize: 13, color: '#a5b4fc', marginBottom: 4 }}>💡 Pro Tip</div>
                  <div style={{ fontSize: 13, color: '#71717a' }}>Check multiple sources for balanced analysis. Consensus can shift before events.</div>
                </div>
              </>)}
            </div>
          </div>
